...
当前 `splits/` 下导出的是 PET 已经使用过的划分，因此不会要求 PET 重新训练：
...
```text
splits/CoNIC/train_ids.txt
splits/CoNIC/val_ids.txt
```
...
```text
splits/MoNuSeg/train_ids.txt
splits/MoNuSeg/val_ids.txt
```
...
```bash
python export_splits.py make-conic --src_root path/to/CoNIC --out_dir splits_new --ratios 0.7,0.15,0.15 --seed 42
```
