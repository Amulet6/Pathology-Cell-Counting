...
class Trainer(object):
    def __init__(self, cfg, model, train_dl, val_dl, criterion):
        ...
        if self.cfg.RESUME and self.cfg.RESUME_PATH:
            ...
            missing_keys = [k for k in model_state.keys() if k not in loadable_state]
            ...
            self.model_without_ddp.load_state_dict(loadable_state, strict=False)
            if missing_keys:
                self.logger.info('Missing keys when loading pretrained weights: %s', missing_keys)
            if unexpected_keys:
                self.logger.info('Unexpected keys in pretrained weights: %s', unexpected_keys)
            if shape_mismatch:
                self.logger.info('Shape-mismatch keys skipped from pretrained weights: %s', shape_mismatch)
...
@torch.no_grad()
def evaluate_crowd_counting(model, data_loader, device, threshold=0.5, vis_dir=None, cfg=None):
    model.eval()
    ...
    with torch.no_grad():
        for samples, targets in data_loader:
            ...
            post_cfg = cfg if cfg is not None else threshold
            points, predict_cnt = utils.get_fused_predictions(outputs, post_cfg, sample_idx=0)
...
@torch.no_grad()
def evaluate_crowd_counting_and_loc(model, data_loader, device, threshold=0.5, vis_dir=None, vis_limit=None, cfg=None):
    model.eval()
