...
def localization_metrics(pred_points, gt_points, radius=8.0):
    ...
    pred = torch.as_tensor(pred_points, dtype=torch.float32)
    gt = torch.as_tensor(gt_points, dtype=torch.float32)
    dist = torch.cdist(pred, gt, p=2)
    matches = []
    for pred_idx in range(dist.shape[0]):
        gt_idx = torch.argmin(dist[pred_idx]).item()
        matches.append((dist[pred_idx, gt_idx].item(), pred_idx, gt_idx))
    matches.sort()

    used_pred, used_gt, tp = set(), set(), 0
    for d, pred_idx, gt_idx in matches:
        if d <= radius and pred_idx not in used_pred and gt_idx not in used_gt:
            used_pred.add(pred_idx)
            used_gt.add(gt_idx)
            tp += 1

    precision = tp / max(len(pred_points), 1)
    recall = tp / max(len(gt_points), 1)
...
def visualization(samples, targets, pred, vis_dir, split_map=None):
    ...
    for idx in range(images.shape[0]):
        ...
        if split_map is not None:
            ...
            sample_vis = np.clip(split_map * 0.9 + sample_vis, 0, 255).astype(np.uint8)
        ...
        if vis_dir is not None:
            ...
            name = os.path.splitext(os.path.basename(targets[idx]['image_path']))[0]
...
# evaluation
@torch.no_grad()
def evaluate(model, data_loader, device, epoch=0, vis_dir=None, loc_radius=24.0):
    model.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        points_abs = (outputs_points * torch.tensor([img_h, img_w], device=outputs_points.device)).detach().cpu().tolist()
        precision, recall, f1 = localization_metrics(points_abs, targets[0]['points'], radius=loc_radius)
        results['precision'] = toTensor(precision)
        results['recall'] = toTensor(recall)
        results['f1'] = toTensor(f1)
        ...
        metric_logger.update(mae=results_reduced['mae'], mse=results_reduced['mse'],
                             precision=results_reduced['precision'], recall=results_reduced['recall'],
                             f1=results_reduced['f1'])

        # visualize predictions
        if vis_dir: 
            points = points_abs     # recover to actual points
