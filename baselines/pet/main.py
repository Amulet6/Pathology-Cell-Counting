...
def get_args_parser():
    ...
    parser.add_argument('--keep_empty_train', default=0, type=int)
    parser.add_argument('--loc_radius', default=24.0, type=float)
    parser.add_argument('--aug_mode', default='native', choices=['native', 'unified'])

    # misc parameters
    parser.add_argument('--output_dir', default='',
                        help='path where to save, empty for no saving')
    parser.add_argument('--output_root', default='./outputs',
                        help='root path where dataset output folders are saved')
...
def main(args):
    ...
    if utils.is_main_process:
        output_dir = os.path.join(args.output_root, args.dataset_file, args.output_dir)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0:
            t1 = time.time()
            test_stats = evaluate(model, data_loader_val, device, epoch, None, loc_radius=args.loc_radius)
            t2 = time.time()
            ...
            print("\nepoch:", epoch, "mae:", mae, "mse:", mse,
                  "precision:", test_stats.get('precision', 0.0),
                  "recall:", test_stats.get('recall', 0.0),
                  "f1:", test_stats.get('f1', 0.0),
                  "\n\nbest mae:", best_mae, "best epoch:", best_epoch)
            print("==========================\n")
            if utils.is_main_process():
                with open(run_log_name, "a") as log_file:
                    log_file.write("\nepoch:{}, mae:{}, mse:{}, precision:{}, recall:{}, f1:{}, time{}, \n\nbest mae:{}, best epoch: {}\n\n".format(
                                                epoch, mae, mse,
                                                test_stats.get('precision', 0.0),
                                                test_stats.get('recall', 0.0),
                                                test_stats.get('f1', 0.0),
                                                t2 - t1, best_mae, best_epoch))
