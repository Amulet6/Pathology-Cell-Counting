...
def main():
    ...
    ap.add_argument("--grid-min", type=float, default=0.10, help="score-threshold grid min (default 0.10)")
    ap.add_argument("--grid-max", type=float, default=0.605, help="score-threshold grid max, exclusive (default 0.605)")
    ap.add_argument("--grid-step", type=float, default=0.05, help="score-threshold grid step (default 0.05)")
    ...
    grid = np.round(np.arange(args.grid_min, args.grid_max, args.grid_step), 2)
    gts, scores = [], []
    with torch.no_grad():
