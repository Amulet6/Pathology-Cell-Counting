#!/usr/bin/env python3
"""
CoNIC Round 1（官方增强）在 overlap4x 数据上重跑。

与 run.py 相同的数据增强策略（仅 flip + blur/noise + 颜色扰动，无 affine scale），
但使用 overlap4x 数据（每原始 patch 固定 4 个子样本）。

数据：exp_output/local/data_overlap/images.npy, labels.npy
分片：exp_output/local/data_overlap/splits.dat
输出：exp_output/local/models/baseline_overlap/
"""
...
if __name__ == "__main__":
    ...
    SAVE_ROOT = f'{WORKSPACE_DIR}/models/baseline_overlap/'
