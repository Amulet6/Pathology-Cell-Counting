...
class RunEngine(object):
    ...
    def run(self, nr_epoch=1, shared_state=None, chained=False, start_epoch=0):
        ...
        self.state.curr_epoch = max(self.state.curr_epoch, start_epoch)
        ...
        while self.state.curr_epoch < nr_epoch:
            ...
            if not self.separate_loader_output:
                ...
                self.__trigger_events(Events.EPOCH_COMPLETED)
                # 早停检查：回调可通过 state._early_stop = True 终止训练
                if getattr(self.state, '_early_stop', False):
                    logging.info('Early stopping triggered — breaking training loop')
                    break
