...
class TriggerEngine(BaseCallbacks):
    ...
    def run(self, state, event):
        ...
        return
####
class EarlyStopping(BaseCallbacks):
    """
    早停回调：挂在 TriggerEngine("infer") 之后，监控验证集 np_dice。
    当验证指标连续 patience 个 epoch 未改善（超过 min_delta）时触发早停。

    Args:
        monitor: 监控的指标名（默认 'infer-valid-0-np_dice'）
        patience: 容忍多少个 epoch 无改善（默认 15）
        min_delta: 最小改善阈值（默认 0.001）
    """
    def __init__(self, monitor='infer-valid-0-np_dice', patience=15, min_delta=0.001):
        super().__init__()
    ...
    def run(self, state, event):
        # TriggerEngine("infer") 刚执行完，stats.dat 已写入当前训练 epoch 的验证指标
        log_file = state.log_info.get('log_file')
        if log_file is None:
            return
        stats = joblib.load(log_file)
        epoch_key = str(state.curr_epoch)
        current_score = stats.get(epoch_key, {}).get(self.monitor, None)

        if current_score is None:
            logging.warning(f'EarlyStopping: monitor "{self.monitor}" not found in stats')
            return

        if self.best_score is None or current_score > self.best_score + self.min_delta:
            self.best_score = current_score
            self.counter = 0
            self.best_epoch = state.curr_epoch
            logging.info(
                f'✅ EarlyStopping: new best {self.monitor}='
                f'{current_score:.5f} @ epoch {state.curr_epoch}')
        else:
            self.counter += 1
            logging.info(
                f'⏳ EarlyStopping: {self.counter}/{self.patience} '
                f'({self.monitor}={current_score:.5f}, '
                f'best={self.best_score:.5f} @ epoch {self.best_epoch})')
            if self.counter >= self.patience:
                logging.info(
                    f'🛑 EarlyStopping: stopping at epoch {state.curr_epoch} '
                    f'(best {self.monitor}={self.best_score:.5f} @ epoch {self.best_epoch})')
