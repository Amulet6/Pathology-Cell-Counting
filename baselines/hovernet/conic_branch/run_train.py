...
class RunManager(object):
    ...
    def _run_once(self, opt, run_engine_opt, log_dir, prev_log_dir=None):
        ...
        if self.logging:
            ...
            # 保留已有 stats（断点续训场景），新 phase 初始化为空
            if os.path.exists(log_file) and os.path.getsize(log_file) > 0:
                existing_stats = joblib.load(log_file)
            else:
                existing_stats = {}
            joblib.dump(existing_stats, log_file)
            log_info = {
                "log_file": log_file,
                "tfwriter": tfwriter,
            }
        ...
        # * 检测断点续训：当前 phase 目录已有 checkpoint 时，从断点恢复
        resume_epoch = 0
        if self.logging and os.path.exists(log_file):
            existing_stats = joblib.load(log_file)
            epoch_keys = [int(v) for v in existing_stats.keys()
                          if str(v).isdigit()]
            if epoch_keys:
                resume_epoch = max(epoch_keys)

        if resume_epoch > 0:
            logging.info(
                f'检测到已有 {resume_epoch} 个 epoch，从 epoch {resume_epoch} 恢复训练'
            )
            # 清除将被重训的 epoch 记录，避免 LoggingEpochOutput 新旧数据冲突
            clean_stats = {}
            removed = 0
            for k in existing_stats:
                if str(k).isdigit() and int(k) >= resume_epoch:
                    removed += 1
                else:
                    clean_stats[k] = existing_stats[k]
            if removed > 0:
                joblib.dump(clean_stats, log_file)
                logging.info(
                    f'清除 {removed} 条过期 epoch 记录（epoch >= {resume_epoch}）')

            for net_name, net_info in net_run_info.items():
                resume_ckpt = os.path.join(
                    log_dir, f'{net_name}_epoch={resume_epoch}.tar')
                if os.path.exists(resume_ckpt):
                    ckpt = torch.load(resume_ckpt)
                    # checkpoint 的 desc 保存为 DataParallel 格式 (带 module. 前缀)，
                    # 此时 net_info['desc'] 已是 DataParallel，直接加载
                    load_feedback = net_info['desc'].load_state_dict(
                        ckpt['desc'], strict=False)
                    logging.info(
                        f'从 {resume_ckpt} 恢复 {net_name} ({resume_epoch} epoch 权重)')
                    # 恢复 optimizer/scheduler 状态（含 momentum 等）
                    if 'optimizer' in ckpt:
                        net_info['optimizer'].load_state_dict(ckpt['optimizer'])
                        logging.info(f'恢复 {net_name} optimizer 状态')
                    if 'lr_scheduler' in ckpt:
                        net_info['lr_scheduler'].load_state_dict(ckpt['lr_scheduler'])
                        logging.info(f'恢复 {net_name} scheduler 状态')

        # start the run loop
        main_runner.run(opt['nr_epochs'], start_epoch=resume_epoch)
    ...
    def run(self):
        ...
        start_phase = getattr(self, 'start_phase', 0)

        prev_save_path = None
        for phase_idx, phase_info in enumerate(phase_list):
            if phase_idx < start_phase:
                # 跳过已完成 phase，但仍需维护 prev_save_path
                if len(phase_list) > 1:
                    prev_save_path = self.log_dir + '/%02d' % phase_idx
                else:
                    prev_save_path = self.log_dir
                logging.info(
                    f'跳过 Phase {phase_idx}（已完成，start_phase={start_phase}）'
                )
                self.phase_idx += 1
                continue
