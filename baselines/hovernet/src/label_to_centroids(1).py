...
def convert_conic_pred(args):
    """CoNIC 预测结果 → 点坐标。seg_preds.npy: (799, 256, 256, 2), ch0=inst_map + 使用 split_csv 获取
    真实 patch_idx 作为 sample id。
    """
    preds = np.load(args.seg_preds_npy, mmap_mode="r")
    if preds.ndim != 4 or preds.shape[-1] != 2:
        raise ValueError(
            f"seg_preds.npy must be [N,H,W,2], got {preds.shape}"
        )

    samples = []
    for i in range(preds.shape[0]):
        inst_map = preds[i, ..., 0].astype(np.int32)
        points = instance_centroids(inst_map)
        # 编号：先用线性编号，后续按 split 过滤时替换为真实 patch_idx
        samples.append({
            "id": f"conic_pred_{i:05d}",
            "points": points,
            "count": len(points)
        })
    ...
    metadata = {
        "dataset": "CoNIC",
        "source_format": "NumPy instance-id masks (prediction)",
        "pred_path": str(Path(args.seg_preds_npy)),
        "coordinate_order": "[x, y]",
        "coordinate_unit": "pixel",
        "centroid_method": "pixel_mean_of_instance_mask",
        "num_samples": len(samples),
        "total_points": int(sum(s["count"] for s in samples))
    }
    write_outputs(args.output, metadata, samples)
...
def build_parser():
    ...
    conic = subparsers.add_parser(
        "conic",
        help="Convert CoNIC GT labels.npy to centroids."
    )
    conic.add_argument("--labels_npy", required=True)
    conic.add_argument("--output", required=True)
    conic.set_defaults(func=convert_conic)

    conic_pred = subparsers.add_parser(
        "conic_pred",
        help="Convert CoNIC PRED seg_preds.npy (inst_map) to centroids."
    )
    conic_pred.add_argument("--seg_preds_npy", required=True)
    conic_pred.add_argument("--output", required=True)
    conic_pred.set_defaults(func=convert_conic_pred)
