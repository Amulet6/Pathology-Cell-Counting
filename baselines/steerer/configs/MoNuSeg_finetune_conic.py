gpus = (0,)
log_dir = 'exp'
workers = 4
print_freq = 30
seed = 3035

network = dict(
    backbone="MocHRBackbone",
    sub_arch='hrnet48',
    counter_type='withMOE',
    resolution_num=[0, 1, 2, 3],
    loss_weight=[1., 1. / 2, 1. / 4, 1. / 8],
    sigma=[4],
    gau_kernel_size=15,
    baseline_loss=False,
    pretrained_backbone="../PretrainedModels/hrnetv2_w48_imagenet_pretrained.pth",

    head=dict(
        type='CountingHead',
        fuse_method='cat',
        in_channels=96,
        stages_channel=[384, 192, 96, 48],
        inter_layer=[64, 32, 16],
        out_channels=1)
)

dataset = dict(
    name='MoNuSeg',
    root='../ProcessedData/MoNuSeg_numeric/',
    test_set='test.txt',
    train_set='train.txt',
    loc_gt='test_gt_loc.txt',
    num_classes=len(network['resolution_num']),
    den_factor=100,
    extra_train_set=None
)

optimizer = dict(
    NAME='adamw',
    BASE_LR=2e-6,
    BETAS=(0.9, 0.999),
    WEIGHT_DECAY=1e-2,
    EPS=1.0e-08,
    MOMENTUM=0.9,
    AMSGRAD=False,
    NESTEROV=True,
)

lr_config = dict(
    NAME='cosine',
    WARMUP_METHOD='linear',
    DECAY_EPOCHS=250,
    DECAY_RATE=0.1,
    WARMUP_EPOCHS=5,
    WARMUP_LR=2.0e-7,
    MIN_LR=1.0e-7
)

log_config = dict(
    interval=50,
    hooks=[
        dict(type='TextLoggerHook'),
    ])

train = dict(
    counter='normal',
    image_size=(640, 640),
    route_size=(160, 160),
    base_size=640,
    batch_size_per_gpu=4,
    shuffle=True,
    begin_epoch=0,
    end_epoch=50,
    extra_epoch=0,
    extra_lr=0,
    resume_path=None,
    pretrained_counter='exp/CoNIC/MocHRBackbone_hrnet48/CoNIC_train_2026-05-29-17-53/Ep_41_mae_5.9217468061181435_mse_8.385865107400564.pth',
    flip=True,
    multi_scale=True,
    scale_factor=(0.5, 1.0 / 0.5),
    val_span=[-1000, -80, -60, -40, -20, -1],
    downsamplerate=1,
    ignore_label=255
)


test = dict(
    image_size=(640, 640),
    base_size=640,
    loc_base_size=640,
    loc_threshold=0.20,
    batch_size_per_gpu=1,
    patch_batch_size=4,
    flip_test=False,
    multi_scale=False,
    point_f1_radii=[8, 16],

    model_file=''
)

CUDNN = dict(
    BENCHMARK=True,
    DETERMINISTIC=False,
    ENABLED=True)
