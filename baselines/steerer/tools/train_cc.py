...
def main():
    ...
    pretrained_counter = getattr(config.train, 'pretrained_counter', None)
    if pretrained_counter:
        if not os.path.isfile(pretrained_counter):
            raise FileNotFoundError(
                'pretrained_counter does not exist: {}'.format(pretrained_counter))
        checkpoint = torch.load(pretrained_counter,
        ...
        if args.local_rank == 0:
            logger.info('=> loaded pretrained counter from {}'.format(pretrained_counter))
