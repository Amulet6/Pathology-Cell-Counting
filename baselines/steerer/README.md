...
> **Repository status:** this directory preserves everything tracked by the
> `steerer-dev` branch. That branch is an integration overlay and does not contain
> the upstream `lib/` or `lib_cls/` core packages imported by the training and
> inference scripts. Apply this directory to a complete upstream STEERER checkout
> before running it; the converters, pathology configs, augmentation tools, and
> experiment commands are retained here.
