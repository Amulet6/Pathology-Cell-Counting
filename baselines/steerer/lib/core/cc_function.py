...
from lib.eval.eval_loc_count import eval_loc_MLE_point, eval_loc_F1_boxes, eval_loc_F1_point
...
def _resize_factor_value(size_factor):
    if hasattr(size_factor, 'detach'):
        size_factor = size_factor.detach().cpu().numpy()
    return float(np.asarray(size_factor).reshape(-1)[0])

def _format_points_line(image_id, points):
    points = _points_array(points)
    items = [str(int(image_id)), str(points.shape[0])]
    for x, y in points:
        items.extend([str(int(round(float(x)))), str(int(round(float(y))))])
    return ' '.join(items)
...
def test_loc(config, test_dataset, testloader, model
    ...
    with torch.no_grad():
        for index, batch in enumerate(tqdm(testloader)):
            ...
            resize_factor = _resize_factor_value(size_factor)
            pred_points_original = _points_array(pred_data['points']) / resize_factor
            gt_points_original = _points_array(gt_data['points'])
            pred_data_4val['points'] = pred_points_original
            pred_data_4val['num'] = pred_points_original.shape[0]
            tp_s, fp_s, fn_s, tp_c_s, fn_c_s, tp_l, fp_l, fn_l, tp_c_l, fn_c_l = eval_loc_F1_boxes(num_classes, pred_data_4val, gt_data)

            tp_100, fp_100, fn_100 = eval_loc_F1_point(pred_points_original, gt_points_original, max_dist_thresh=max_dist_thresh)
            Distance_Sum = eval_loc_MLE_point(pred_points_original, gt_points_original, 16)
            pred_point_lines.append(_format_points_line(name[0], pred_points_original))
        ...
        if sv_dir:
            pred_points_file = os.path.join(sv_dir, 'pred_points.txt')
            with open(pred_points_file, 'w') as f:
                f.write('\n'.join(pred_point_lines))
                if pred_point_lines:
                    f.write('\n')
            logging.info('Saved point predictions: {}'.format(pred_points_file))
        ...
        point_radii = getattr(config.test, 'point_f1_radii', [8, 16])
        for radius in point_radii:
            radius_idx = int(radius) - 1
            if 0 <= radius_idx < max_dist_thresh:
                logging.info(
                    'radius_{}px: precision={}, recall={}, F1={}'.format(
                        radius,
                        pre_100[radius_idx],
                        rec_100[radius_idx],
                        f1_100[radius_idx]))
