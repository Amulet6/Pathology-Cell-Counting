...
- `eval.py`：测试/评估入口。
- `engine.py`：训练与评估循环。
- `prepare_cell_dataset.py`：BCData、CoNIC、MoNuSeg 等数据转换脚本。
- `export_splits.py`：统一划分导出脚本。
- `splits/`：小组统一划分文件。
- `CELL_REPRO.md`、`SERVER_RUN_COMMANDS.md`：复现实验说明与服务器运行命令。
- `coordinate_protocol.json`、`pet_data_augmentation_protocol.json`：坐标与增强协议。
...
```bash
python prepare_cell_dataset.py --dataset bcdata \
...
```bash
python prepare_cell_dataset.py --dataset monuseg \
...
```bash
python prepare_cell_dataset.py --dataset conic_split \
...
```bash
python main.py --dataset_file CoNIC \
...
```bash
python eval.py --dataset_file CoNIC \
  --data_path data/CoNIC_pet_split \
  --device cuda \
  --num_workers 2 \
