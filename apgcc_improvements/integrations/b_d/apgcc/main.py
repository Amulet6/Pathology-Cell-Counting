...
def test(cfg):
    ...
    weight_path = cfg.TEST.WEIGHT if cfg.TEST.WEIGHT else os.path.join(source_dir, 'best.pth')
    pretrained_dict = torch.load(weight_path, map_location='cpu')
    state_dict = pretrained_dict['model'] if isinstance(pretrained_dict, dict) and 'model' in pretrained_dict else pretrained_dict
    model.load_state_dict(state_dict, strict=False)

    # Starting Testing
    print("Start testing")
    t1 = time.time()
    result = evaluate_crowd_counting_and_loc(model, val_dl, next(model.parameters()).device, cfg.TEST.THRESHOLD, vis_val_path, vis_limit=5)
    t2 = time.time()
    mae, mse, loc = result[0], result[1], result[2]
    logger.info('Eval: MAE=%.6f, MSE=%.6f time:%.2fs' % (mae, mse, t2 - t1))
    logger.info('Localization (σ=4): Prec=%.4f  Recall=%.4f  F1=%.4f' % (loc['ap_4'], loc['ar_4'], loc['f1_4']))
    logger.info('Localization (σ=8): Prec=%.4f  Recall=%.4f  F1=%.4f\n' % (loc['ap_8'], loc['ar_8'], loc['f1_8']))
