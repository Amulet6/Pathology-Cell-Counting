...
class ImageDataset(Dataset):
    def __init__(self, data_root, transform=None, train=False, aug_dict=None, eval_list="test.list"):
        ...
        # ===== Direction-E: pathology / stain augmentation (train-only preprocessing) =====
    ...
    def __getitem__(self, index):
        ...
        # ===== Direction-E: stain/pathology augmentation (train-only, before any geometric aug) =====
        domain_label = get_domain_label(img_path, self.domain_names)
        if self.train and self.pathology_aug:
            img = pathology_augment(img, stain_jitter=self.stain_jitter, blur_noise=self.blur_noise)
        ...
        if self.train and self.unified is not None:
            ...
            for i, _ in enumerate(point):
                ...
                target[i]['crop_size'] = (self.crop_size, self.crop_size)
        ...
        for i, _ in enumerate(point):
            ...
            if self.train:
                ...
                # eval: full image (possibly downscaled), [C, H, W]
                _, h, w = img.shape if img.dim() == 3 else img[0].shape
                target[i]['crop_size'] = (h, w)
        return img, target # target: {'point', 'image_id', 'labels', 'name', 'crop_size', 'domain'}
...
# ===== Direction-E: stain / pathology augmentation helpers =====
def get_domain_label(img_path, domain_names):
    """Infer the CoNIC source-domain index from the file stem (crag_*, dpath_*, ...)."""
...
def pathology_augment(img, stain_jitter=False, blur_noise=False):
    """Light pathology-stain robustness augmentation on a PIL RGB image (train only)."""
    if stain_jitter:
        img = ImageEnhance.Brightness(img).enhance(random.uniform(0.85, 1.15))
        img = ImageEnhance.Contrast(img).enhance(random.uniform(0.85, 1.15))
        img = ImageEnhance.Color(img).enhance(random.uniform(0.85, 1.15))
        arr = np.asarray(img).astype(np.float32)
        channel_scale = np.random.uniform(0.92, 1.08, size=(1, 1, 3)).astype(np.float32)
        channel_bias = np.random.uniform(-6.0, 6.0, size=(1, 1, 3)).astype(np.float32)
        arr = np.clip(arr * channel_scale + channel_bias, 0, 255).astype(np.uint8)
    ...
    if blur_noise:
        ...
        if random.random() < 0.15:
            arr = np.asarray(img).astype(np.float32)
            noise = np.random.normal(0.0, random.uniform(2.0, 6.0), size=arr.shape).astype(np.float32)
