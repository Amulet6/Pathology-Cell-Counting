...
class SetCriterion_Crowd(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, aux_kwargs,
                 edge_band=0, edge_weight=0.0, focal_gamma=0.0):
        ...
        Parameters:
            ...
            focal_gamma: Direction-A softmax-focal modulation exponent on the proposal cls loss;
                         0.0 == plain weighted CE (composes with edge masking below)
        """
        super().__init__()
    ...
    def loss_labels(self, outputs, targets, indices, num_points):
        ...
        # Per-anchor classification loss (reduction='none') so it composes with edge masking.
        # Direction-A softmax-focal: -alpha_t * (1-p_t)^gamma * log(p_t), alpha = empty_weight.
        # Reduces EXACTLY to the weighted CE below when focal_gamma == 0.
        if self.focal_gamma and self.focal_gamma > 0:
            logp = F.log_softmax(src_logits, dim=-1)                            # [B, N, 2]
            logpt = logp.gather(-1, target_classes.unsqueeze(-1)).squeeze(-1)   # [B, N]
            pt = logpt.exp()
            alpha = self.empty_weight[target_classes]                          # [B, N]
            loss_per_anchor = -alpha * ((1.0 - pt) ** self.focal_gamma) * logpt  # [B, N]
        else:
        ...
        # Keep Direction-D's normalization (sum / num_points) so loss_ce and the edge-masked
        # loss_points stay on a consistent scale on this combined base.
