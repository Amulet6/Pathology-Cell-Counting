...
_C.MODEL.LOSS = ['L2']
_C.MODEL.WEIGHT_DICT = {'loss_ce': 1, 'loss_points': 0., 'loss_aux': 0., 'loss_dense': 0.}
...
_C.MODEL.DENSE_AUX_EN = False
_C.MODEL.DENSE_AUX_LEVEL = 3
_C.MODEL.DENSE_SIGMA = 2.0

# DCNv2 (Deformable Convolution v2)
_C.MODEL.DCNV2_ENABLED = False   # enable DCNv2 in the backbone conv5
_C.MODEL.DCNV2_LAYERS = 2        # number of conv5 layers to replace (1 or 2)

# Edge Ignore – anchor-level edge masking
_C.MODEL.EDGE_BAND = 0           # edge ignore band width (px), 0 = disabled
_C.MODEL.EDGE_WEIGHT = 0.0       # loss weight for anchors inside the edge band (0~1)

# Direction C: optional repulsion loss, kept as an ablation switch.
_C.MODEL.REPULSION = edict()
_C.MODEL.REPULSION.ENABLED = False
_C.MODEL.REPULSION.WEIGHT = 0.05
_C.MODEL.REPULSION.RADIUS = 8.0
_C.MODEL.REPULSION.GT_SAFE_DISTANCE = 6.0

_C.RESUME = False      # resume, resume from checkpoint
_C.RESUME_PATH = ''   # path to checkpoint (used when RESUME=True)
_C.FINETUNE = False   # if True, load weights only (skip optimizer/epoch restore)
...
_C.TEST.WEIGHT = ""
_C.TEST.EDGE_BAND = 0            # edge-aware filtering band (px), 0 = disabled
_C.TEST.ADAPTIVE_NMS = edict()
_C.TEST.ADAPTIVE_NMS.ENABLED = False
_C.TEST.ADAPTIVE_NMS.DENSE_RADIUS = 4.0
_C.TEST.ADAPTIVE_NMS.SPARSE_RADIUS = 8.0
_C.TEST.ADAPTIVE_NMS.DENSE_KNN_DIST = 10.0
