...
class SetCriterion_Crowd(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, aux_kwargs,
        ...
        self.repulsion_kwargs = repulsion_kwargs or {}
    ...
    def loss_repulsion(self, outputs, targets, indices, num_points):
        """Penalize duplicated positive predictions collapsing into one cell."""
        assert 'pred_points' in outputs
        radius = float(self.repulsion_kwargs.get('RADIUS', 8.0))
        gt_safe_distance = float(self.repulsion_kwargs.get('GT_SAFE_DISTANCE', 6.0))
        if radius <= 0:
            return {'loss_repulsion': outputs['pred_points'].sum() * 0.0}

        rep_losses = []
        for batch_idx, (src_idx, tgt_idx) in enumerate(indices):
            if src_idx.numel() < 2:
                continue
            pred_points = outputs['pred_points'][batch_idx, src_idx]
            gt_points = targets[batch_idx]['point'][tgt_idx]

            pred_dist = torch.cdist(pred_points, pred_points, p=2)
            gt_dist = torch.cdist(gt_points, gt_points, p=2)
            pair_mask = torch.triu(torch.ones_like(pred_dist, dtype=torch.bool), diagonal=1)
            pair_mask = pair_mask & (gt_dist >= gt_safe_distance)
            if not pair_mask.any():
                continue

            close_dist = pred_dist[pair_mask]
            rep_losses.append((F.relu(radius - close_dist) / radius).pow(2).mean())

        if not rep_losses:
            return {'loss_repulsion': outputs['pred_points'].sum() * 0.0}
        return {'loss_repulsion': torch.stack(rep_losses).mean()}
    ...
    def forward(self, outputs, targets, show=False):
        ...
        for loss in self.weight_dict.keys():
            ...
            elif loss == 'loss_repulsion':
                losses.update(self.loss_repulsion(output1, targets, indices1, num_boxes))
            else:
