...
def adaptive_point_nms(points, scores, cfg):
    """Score-sorted point NMS with density-aware suppression radius."""
    if points.numel() == 0:
        return torch.empty(0, dtype=torch.long, device=points.device)

    nms_cfg = cfg.TEST.ADAPTIVE_NMS
    dense_radius = float(nms_cfg.DENSE_RADIUS)
    sparse_radius = float(nms_cfg.SPARSE_RADIUS)
    dense_knn_dist = float(nms_cfg.DENSE_KNN_DIST)
    order = torch.argsort(scores, descending=True)

    keep = []
    suppressed = torch.zeros(points.shape[0], dtype=torch.bool, device=points.device)
    if points.shape[0] > 1:
        dist_all = torch.cdist(points, points, p=2)
        masked = dist_all + torch.eye(points.shape[0], device=points.device) * 1e6
        local_dist = masked.min(dim=1).values
    else:
        local_dist = torch.full((1,), sparse_radius + 1.0, device=points.device)

    for idx in order:
        if suppressed[idx]:
            continue
        keep.append(idx)
        radius = dense_radius if local_dist[idx] <= dense_knn_dist else sparse_radius
        dist = torch.norm(points - points[idx], dim=1)
...
def filter_prediction_points(outputs_points, outputs_scores, threshold, cfg=None,
                             samples=None, edge_band=0):
    """Apply score threshold, optional Direction D edge filter, then Direction C NMS."""
    keep = outputs_scores > threshold
    if edge_band > 0 and samples is not None:
        from util.misc import edge_aware_filter
        _, _, H, W = samples.shape
        global_shape = (int(H), int(W))
        patch_box = (0, 0, int(W), int(H))
        keep = edge_aware_filter(outputs_points, outputs_scores,
                                 global_shape, patch_box,
                                 edge_band, threshold)

    points = outputs_points[keep]
    scores = outputs_scores[keep]
    nms_cfg = getattr(getattr(cfg, 'TEST', None), 'ADAPTIVE_NMS', None) if cfg is not None else None
    if nms_cfg is not None and nms_cfg.ENABLED and points.shape[0] > 0:
        nms_keep = adaptive_point_nms(points, scores, cfg)
        points = points[nms_keep]
...
class Trainer(object):
    ...
    def step(self, batch):
        ...
        targets = [{k: v.to(device) if k not in ('name', 'crop_size') else v for k, v in t.items()} for t in targets]
        # forward
...
def evaluate_crowd_counting(model, data_loader, device, threshold=0.5, vis_dir=None,
    ...
    with torch.no_grad():
        for samples, targets in data_loader:
            ...
            points_tensor, _ = filter_prediction_points(outputs_points, outputs_scores,
                                                        threshold, cfg,
                                                        samples=samples,
                                                        edge_band=edge_band)
            points = points_tensor.detach().cpu().numpy().tolist()
            predict_cnt = int(points_tensor.shape[0])
...
@torch.no_grad()
def evaluate_crowd_counting_and_loc(model, data_loader, device, threshold=0.5,
                                    vis_dir=None, vis_limit=None, cfg=None):
    model.eval()
    ...
    for ct, (samples, targets) in enumerate(data_loader):
        ...
        gt_cnt = targets[0]['point'].shape[0]
        edge_band = cfg.TEST.get('EDGE_BAND', 0) if cfg is not None else 0
        points_tensor, _ = filter_prediction_points(outputs_points, outputs_scores,
                                                    threshold, cfg,
                                                    samples=samples,
                                                    edge_band=edge_band)
