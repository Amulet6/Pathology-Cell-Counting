...
class DomainClassifier(nn.Module):
    def __init__(self, in_channels, hidden_dim=256, num_classes=5):
        super().__init__()
...
class Model_builder(nn.Module):
    def __init__(self, cfg):
        ...
        self.domain_adaptation = bool(getattr(cfg.MODEL, 'DOMAIN_ADAPTATION', False))
        self.domain_grl_lambda = getattr(cfg.MODEL, 'DOMAIN_GRL_LAMBDA', 1.0)
        if self.domain_adaptation:
            self.domain_classifier = DomainClassifier(
                in_channels=self.encoder.get_outplanes()[-1],
                hidden_dim=getattr(cfg.MODEL, 'DOMAIN_HIDDEN_DIM', 256),
                num_classes=getattr(cfg.MODEL, 'DOMAIN_NUM_CLASSES', 5),
            )
    ...
    def forward(self, samples: NestedTensor):
        ...
        if self.domain_adaptation:
            out['domain_logits'] = self.domain_classifier(features[-1], self.domain_grl_lambda)
        return out   # {'pred_logits', 'pred_points', 'offset'}

class SetCriterion_Crowd(nn.Module):
    # Copyright (C) 2021 THL A29 Limited, a Tencent company.  All rights reserved. 
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, aux_kwargs, domain_adaptation=False):
    ...
    def loss_domain(self, outputs, targets):
        if 'domain_logits' not in outputs:
            return {'loss_domain': outputs['pred_logits'].sum() * 0.0}
        domain_targets = torch.cat([t['domain'] for t in targets], dim=0).to(outputs['domain_logits'].device)
        return {'loss_domain': F.cross_entropy(outputs['domain_logits'], domain_targets)}
    ...
    def forward(self, outputs, targets, show=False):
        ...
        for loss in self.weight_dict.keys():
            ...
            elif loss == 'loss_domain':
                losses.update(self.loss_domain(outputs, targets))
            else:
