...
def main():
    ...
    with torch.no_grad():
        for samples, targets in val_dl:
            ...
            # Edge-aware filter: suppress predictions near internal patch seams
            if args.edge_band > 0:
                from util.misc import edge_aware_filter
                _, _, H, W = samples.shape
                global_shape = (int(H), int(W))
                patch_box = (0, 0, W, H)  # full-image eval: all borders are global
                keep = edge_aware_filter(pts, scores, global_shape, patch_box,
                                         args.edge_band, args.score_threshold)

            pred_pts = pts[keep].detach().cpu().numpy().tolist()
