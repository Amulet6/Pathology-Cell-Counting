...
所有命令默认在本目录的 **`apgcc/`** 子目录下执行（配置文件里用的是 `./output`、`./configs` 等相对路径，且脚本 `import config`，必须在该目录运行）。
...
- 统一增强协议的完整定义见 **`../data_augmentation_protocol.md`**（实现：`apgcc/datasets/aug_unified.py`）。
- 方向 B dense loss-only 改进说明见 **`APGCC_direction_B_dense_loss_report.md`**。
