...
def evaluate(gt_path, pred_path, thresholds):
    ...
    if missing_pred_ids:
        print(f"WARNING: {len(missing_pred_ids)} GT samples have no predictions; treating them as empty predictions.")
        print(f"  Missing pred IDs (first 5): {missing_pred_ids[:5]}")
    if extra_pred_ids:
        print(f"WARNING: {len(extra_pred_ids)} prediction samples are not in GT; ignoring them.")
    ...
    for sid in sample_ids:
        gt_pts = gt_dict[sid]
        pred_pts = pred_dict.get(sid, np.empty((0, 2), dtype=np.float64))
